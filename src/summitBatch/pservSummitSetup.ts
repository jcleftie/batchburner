...
export async function main(ns: NS) {
  ...
  const filesToCopy = [
    'summitBatch/summitBatcher.js',
    'summitBatch/growHelper.js',
    'summitBatch/hackHelper.js',
    'summitBatch/weakenHelper.js',
  ];

  // List of batcher scripts and their respective targets
  const batchers = [
    { script: 'summitBatch/summitBatcher.js', target: 'summit-uni' },

  ];

  const purchasedServers = ns.getPurchasedServers();
  const targetRamUtilization = 0.85; // Utilize up to 70% of available RAM
  const maxBatcherThreads = 10000; // Cap the number of threads per batcher to prevent runaway allocation
